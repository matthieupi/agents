 ...
 * `/sub <task>` starts a new subagent, while `/sub <id> <prompt>` continues an
 * existing one. `/subcont` remains available as a compatibility alias.
 *
 * Usage: pi -e extensions/subagent-widget.ts
 * Then:
 *   /sub list files and summarize          — spawn a new subagent
 *   /sub 1 now write tests for it          — continue subagent #1's conversation
...
import { requireLocalProjectExecution } from "../extension-library/remote-project-mode.ts";
...
export default function (pi: ExtensionAPI) {
	...
	// ── /sub <task> or /sub <number> <prompt> ────────────────────────────────

	pi.registerCommand("sub", {
		description: "Spawn or continue a subagent: /sub <task> or /sub <number> <prompt>",
		handler: async (args, ctx) => {
			widgetCtx = ctx;

			const trimmed = args?.trim() ?? "";
			if (!trimmed) {
				ctx.ui.notify("Usage: /sub <task> or /sub <number> <prompt>", "error");
				return;
			}

			const firstSpace = trimmed.indexOf(" ");
			const firstToken = firstSpace === -1 ? trimmed : trimmed.slice(0, firstSpace);
			const maybeId = parseInt(firstToken, 10);
			const continuationPrompt = firstSpace === -1 ? "" : trimmed.slice(firstSpace + 1).trim();
			const isContinuation = !isNaN(maybeId) && continuationPrompt.length > 0;

			if (isContinuation) {
				const state = agents.get(maybeId);
				if (!state) {
					ctx.ui.notify(`No subagent #${maybeId} found. Use /sub to create one.`, "error");
					return;
				}

				if (state.status === "running") {
					ctx.ui.notify(`Subagent #${maybeId} is still running — wait for it to finish first.`, "warning");
					return;
				}
				...
				ctx.ui.notify(`Continuing Subagent #${maybeId} (Turn ${state.turnCount})…`, "info");
				spawnAgent(state, continuationPrompt, ctx);
				return;
			}
	...
	pi.registerCommand("subcont", {
		description: "Continue an existing subagent's conversation: /subcont <number> <prompt> (alias: /sub <number> <prompt>)",
		handler: async (args, ctx) => {
			widgetCtx = ctx;
			...
			if (spaceIdx === -1) {
				ctx.ui.notify("Usage: /subcont <number> <prompt> or /sub <number> <prompt>", "error");
				return;
			}
