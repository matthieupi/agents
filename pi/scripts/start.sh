...
start_main() {
    ...
    case "$mode" in
        service)
            ...
            [[ -x "$PI_PREFIX/bin/pi-web" ]] || die 'install the runtime first'
            ;;
        session) [[ -x "$PI_PREFIX/bin/pi" ]] || die 'install the runtime first' ;;
    ...
    [[ "$PI_WORKSPACE/" != "$PI_ROOT/"* ]] || die 'workspace must not be inside the Pi component/runtime'
    cd -- "$PI_WORKSPACE"
    case "$mode" in
