...
function normalizeId(value: string): string {
	...
	if (!/^[A-Za-z0-9][A-Za-z0-9_-]{0,127}$/.test(id)) {
		throw new Error("Invalid delegation or session ID")
	}
	return id
}
...
class DelegationManager {
	...
	private async getDelegationsDir(sessionID: string): Promise<string> {
		...
		return path.join(this.baseDir, normalizeId(rootID))
	}
	...
	private async ensureDelegationsDir(sessionID: string): Promise<string> {
		...
		if ((await fs.realpath(dir)) !== path.resolve(dir)) {
			throw new Error("Symlinked delegation directory refused")
		}
		return dir
	}
	...
	private async readPersistedArtifact(filePath: string): Promise<string | null> {
		...
		if (relative.startsWith("..") || path.isAbsolute(relative)) {
			throw new Error("Delegation artifact escaped storage")
		}
		try {
			if ((await fs.lstat(filePath)).isSymbolicLink()) {
				throw new Error("Symlinked delegation artifact refused")
			}
			...
			try {
				if (!(await file.stat()).isFile()) throw new Error("Regular delegation artifact required")
